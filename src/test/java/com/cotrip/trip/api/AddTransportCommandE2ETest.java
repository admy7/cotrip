...
public class AddTransportCommandE2ETest {
  ...
  void setUp() {
    ...
    var trip = new Trip("Paris", "London", "2022-12-03", "2022-12-08");
  ...
  void shouldAddTransportToTrip() throws Exception {
    var dto =
        new AddTransportDTO(
            ...
            new AddPlaceDTO(
                ...
                "SW1W 9TP",
                "United Kingdom"),
            new AddPriceDTO("GBP", 34));

    var response =
        mockMvc
            .perform(
                MockMvcRequestBuilders.post(String.format("/trips/%s/add-transport", tripId))
    ...
    Assertions.assertEquals(
        "Victoria Coach Station, 164 Buckingham Palace Rd", transport.getPlace().getAddress());
    ...
    Assertions.assertEquals(
        0, BigDecimal.valueOf(34.0).compareTo(transport.getPrice().getAmount()));
  }
  ...
  void whenTripDoesNotExist_ShouldSendNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/trips/non-existing-trip-id/add-transport")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
